...
\subsection{Reproducibilidad y artefactos de referencia}
\label{subsec:eval-repo-video}

Para garantizar la trazabilidad académica del experimento, se publica el código fuente completo del prototipo en \href{https://github.com/ljpcastroc/arlequin}{https://github.com/ljpcastroc/arlequin}. La demostración end-to-end del pipeline (detección de \textit{drift} $\rightarrow$ disparo automático de Jenkins $\rightarrow$ reentrenamiento y recuperación) se documenta en video en \href{https://youtu.be/veM20n46DP8}{https://youtu.be/veM20n46DP8}. Ambos recursos permiten replicar la configuración (\texttt{docker-compose}), los scripts de generación/monitoreo y la secuencia de ejecución mostrada en este capítulo.
...
El énfasis en F1 y AUC responde a que el caso de estudio es un clasificador de fraude sobre \textbf{transacciones bancarias sintéticas}; se monitorea la capacidad de detectar movimientos sospechosos y evitar falsas alarmas. La arquitectura del pipeline y del \textit{drift-watcher} es independiente del tipo de tarea: la detección se basa únicamente en KS, $\chi^2$ y PSI, y el retrigger invoca el \texttt{JENKINS\_JOB} configurado. Para un modelo de regresión se sustituirían las métricas de validación por MAE/RMSE (o cualquier criterio de interés) y se configuraría \texttt{SCORE\_COL}/\texttt{POSITIVE\_THRESHOLD} para exponer la variable continua de salida, sin modificar la lógica de monitoreo. En casos de uso distintos (p.\,ej., demanda, mantenimiento predictivo, IoT), la interpretación de las alertas y de \texttt{predicted\_positive\_ratio} debe alinearse con el KPI operativo relevante y con las políticas de riesgo del dominio, pero el ciclo detectar $\rightarrow$ reentrenar $\rightarrow$ auditar permanece igual.

Dentro del conjunto de métricas supervisadas (Precisión, Recall, FPR, etc.), \textbf{F1 y AUC} se seleccionaron como criterios principales porque el dominio de fraude presenta \textbf{clases altamente desbalanceadas} y costos distintos para falsos positivos/negativos. El F1 asegura que la recuperación del desempeño considere simultáneamente precisión y recall—un descenso en cualquiera de las dos afectaría el indicador—mientras que el AUC captura la calidad del ranking probabilístico antes de aplicar umbrales. El \textbf{recall} individual se monitorea y reporta en MLflow, pero no se usa como métrica primaria ya que maximizarlo aisladamente provocaría un aumento de falsos positivos y costos operativos (más bloqueos manuales), comprometiendo el SLO de intervención humana. Por ello, F1/AUC funcionan como métricas de aceptación, y el recall queda documentado como señal secundaria para diagnósticos.
...
\midrule
Job de evaluación  & $16$ vs. $24$ & $2.55\times10^{-3}$ & $0.57$ (grande) \\
Drift-watcher & $16$ vs. $22$ & $5.07\times10^{-3}$ & $0.54$ (grande) \\
\bottomrule
\end{tabular}
\\[2pt]
...
\paragraph{Procedimiento iterativo.}
Cada ejecución experimental siguió el mismo ciclo controlado (aplicado más de 40 veces) y registrado en \texttt{/metrics\_test}:
\begin{enumerate}\setlength\itemsep{1pt}
  \item \textbf{Ingesta controlada.} Airflow ejecutó el ETL \texttt{generate\_data\_session.py}, escribiendo una nueva partición en HDFS y parametrizando \texttt{drift\_factor} ($0$ sin drift, $1$ con drift) para alternar escenarios en iteraciones consecutivas.
  \item \textbf{Monitoreo y detección.} El \textit{drift-watcher} leyó las particiones más recientes dentro de su ventana móvil de 5\,min (\texttt{WINDOW\_MIN}), calculó KS/$\chi^2$/PSI y mantuvo un \textbf{TTFD} (tiempo de detección) sub-minutal. Cuando \texttt{drift\_factor}=1 se esperaba a que el watcher declarara una alerta y, con ella, disparara automáticamente el job \texttt{retrain-model} de Jenkins.
  \item \textbf{Evaluación sin drift.} Cuando \texttt{drift\_factor}=0, se ejecutó el segundo pipeline de Jenkins (\texttt{eval-model}) en modo \textit{pre}/\textit{post} para medir el estado del modelo sin intervención del watcher, consolidando las métricas de referencia.
  \item \textbf{Reentrenamiento y cierre.} Tras cada trigger (manual o automático), el job correspondiente entrenó/validó el modelo, publicó métricas (F1, Recall, AUC, PSI) y artefactos en MLflow, mientras los CSVs de \texttt{metrics\_test} capturaban la corrida específica (eval\_* para el pipeline manual, watcher\_* para el automático).
  \item \textbf{Nueva iteración.} Finalizado el ciclo, el ETL se re-ejecutó alternando el valor de \texttt{drift\_factor} para volver a insertar datos y repetir la secuencia bajo la otra condición.
\end{enumerate}

Este procedimiento garantiza pares consecutivos de ingesta–detección–reentrenamiento con ventanas temporales explícitas: los \textbf{TTFD} (~270\,s mediana) cuantifican el tiempo entre la llegada de datos y la alerta del watcher, mientras que los \textbf{TTR} (~102\,s mediana) corresponden al tiempo del pipeline de Jenkins desde el disparo hasta el registro en MLflow. Ambas métricas provienen de los logs de \texttt{metrics\_test} y respaldan la estabilidad temporal del ciclo detectar $\rightarrow$ actuar $\rightarrow$ verificar.
...
\paragraph{Definición operativa de TTFD y TTR.}
\textbf{TTFD (Time To First Detection)} es el intervalo entre la última observación etiquetada como “normal” y la primera corrida que activa la regla $p<\alpha$ o $\mathrm{PSI}>\tau$; se estima a partir de \texttt{training\_log.csv} siguiendo la noción de latencia de detección descrita en \cite{Sethi2017,Chawla2021}.\\
\textbf{TTR (Time To Recovery)} corresponde al intervalo entre la detección y la finalización del reentrenamiento automático (etapas \texttt{post} en Jenkins), análogo al \textit{mean time to recovery} en MLOps/DevOps \citep{Amershi2019}. Ambos KPIs se reportan como medianas (TTFD$_{50}$, TTR$_{50}$) y sustentan las hipótesis H1 y H3.
...
\begin{enumerate}
  ...
  \item \textbf{Ablación–KS:} se deshabilita la prueba de Kolmogorov–Smirnov para cuantificar la pérdida de sensibilidad en variables numéricas. El objetivo es medir cuánto aporta el contraste continuo (media/forma de la distribución) frente a operar sólo con PSI/$\chi^2$, observando variaciones en \textbf{TTFD} y tasa de falsos negativos.
...
\begin{enumerate}
  ...
  Estos experimentos permiten identificar el punto de operación óptimo entre sensibilidad y estabilidad, configurando un equilibrio adecuado para los SLO-1 y SLO-3..
\end{enumerate}
...
\addplot[accent, mark=*, thick] coordinates {(0.001,0.2) (0.01,0.8) (0.05,3.1)};
\end{axis}
\end{tikzpicture}
...
\addplot[dark, mark=*, thick] coordinates {(500,180) (1000,270) (1500,330)};
\end{axis}
\end{tikzpicture}
...
El análisis estadístico se diseñó para cuantificar la confiabilidad de los resultados y contrastar las hipótesis planteadas en la Sección~\ref{subsec:eval-rq}. Se optó por métodos no paramétricos y procedimientos de remuestreo debido a que (i) las métricas como F1 y PSI provienen de muestras moderadas ($n\leq 24$) con distribuciones potencialmente no normales/sesgadas, (ii) los registros de TTFD/TTR contienen valores atípicos y colas largas, y (iii) la combinación de condiciones pre/post no garantiza homocedasticidad. Bajo estas circunstancias, técnicas como \textit{bootstrap}, Mann–Whitney o DeLong aportan estimaciones robustas sin exigir supuestos fuertes de normalidad. Todas las pruebas se realizaron con un nivel de significancia $\alpha = 0.05$ salvo indicación contraria.
...
\begin{table}[htbp]
\centering
\caption{Métricas consolidadas por escenario (F1 y PSI).}
...
\paragraph{Comparación porcentual.}
La caída de F1 entre E1 y E2 es de \(\frac{0.811-0.825}{0.825}\approx -1.7\,\%\); tras el reentrenamiento, E3 queda a \(\frac{0.817-0.825}{0.825}\approx -1.0\,\%\) de la línea base y mejora \(\frac{0.817-0.811}{0.811}\approx +0.7\,\%\) frente al estado degradado. El PSI disminuye de $0.0067$ a $0.0028$ (\(\approx -58\,\%\)), señalando la corrección del cambio de distribución. Las latencias se mantienen prácticamente constantes: TTFD pasa de 270\,s a 271\,s (+0.4\,\%) y TTR de 102\,s a 103\,s (+1.0\,\%), lo que indica que la recuperación no penaliza el tiempo de respuesta.
Estas diferencias porcentuales deben leerse junto con los IC/efectos ya reportados (Tabla~\ref{tab:scenario-metrics} y Tabla~\ref{tab:mw-tests}); los tamaños de efecto (Cliff $\delta$) y los IC por bootstrap evitan sobre-interpretar variaciones pequeñas en muestras moderadas.
...
\paragraph{Flexibilidad de la métrica de desempeño.}
Si bien la validación de los resultados se ha fundamentado en el F1-score debido a su pertinencia para ponderar precisión y exhaustividad en conjuntos desbalanceados (como es típico en detección de fraude), es importante aclarar que la arquitectura del sistema es agnóstica a la métrica seleccionada. El diseño implementado es completamente parametrizable, lo que significa que el \textit{trigger} de reentrenamiento y los criterios de aceptación pueden configurarse para optimizar otras métricas según las necesidades del negocio, tales como la sensibilidad (\textit{recall}), la precisión o el AUC, sin alterar la lógica operativa del flujo MLOps.
...
\addplot[dark, dashed] coordinates {(0,0) (1,1)}; \addlegendentry{Aleatorio}

% ROC PRE (AUC ≈ 0.79)
\addplot[thick, dark] coordinates {
...
\addplot[thick, dark, dotted] coordinates {
...
\addplot[ybar, fill=accent!35, draw=accent] coordinates {(Eval,102) (Watcher,105)};
\end{axis}
\end{tikzpicture}
...
\addplot[ybar, fill=accent!30, draw=accent] coordinates {(1,0.22) (2,0.21) (3,0.20) (4,0.19) (5,0.18)};
\addlegendentry{Referencia $r_k$}
\addplot[ybar, fill=dark!30, draw=dark] coordinates {(1,0.12) (2,0.18) (3,0.22) (4,0.24) (5,0.24)};
...
\addplot[thick, accent] coordinates{(0,0) (0.1,0.08) (0.2,0.20) (0.3,0.35) (0.4,0.52) (0.5,0.68) (0.6,0.80) (0.7,0.90) (0.8,0.96) (0.9,0.99) (1,1)};
\addlegendentry{CDF referencia}
% ECDF corriente (desplazada)
\addplot[thick, dark, dotted] coordinates{(0,0) (0.1,0.04) (0.2,0.12) (0.3,0.25) (0.4,0.42) (0.5,0.60) (0.6,0.74) (0.7,0.86) (0.8,0.94) (0.9,0.98) (1,1)};
...
\addplot[thick, accent] coordinates{(1,0.45) (2,0.50) (3,0.40) (4,0.35) (5,0.20) (6,0.08) (7,0.02) (8,0.006) (9,0.03) (10,0.08) (11,0.20) (12,0.30) (13,0.40) (14,0.50)};
\addlegendentry{$p_{\min}$}

% PSI (reescalado)
\addplot[thick, dark, dashed] coordinates{(1,0.05) (2,0.05) (3,0.06) (4,0.07) (5,0.12) (6,0.18) (7,0.21) (8,0.23) (9,0.19) (10,0.16) (11,0.12) (12,0.10) (13,0.08) (14,0.07)};
\addlegendentry{PSI}
...
disparos alineados con los cruces de umbral.
...
\begin{itemize}
  ...
  \item \textbf{Parámetros de entorno:} Algunos de los parámetros de ejecución utilizados fueron:\\
  {\small
  \texttt{DRIFT\_ALPHA}=0.01, \texttt{PSI\_ALERT}=0.2, \texttt{SAMPLE\_MAX}=1000, \texttt{LOOP\_SECONDS}=30,\\
  \texttt{DRIFT\_COOLDOWN\_SECONDS}=300, \texttt{DRIFT\_CLEAR\_STREAK}=3, \texttt{TRIGGER\_EDGE\_ONLY}=1.
  }\normalsize
...
\begin{tabular}{p{4cm} p{4cm} p{5.2cm}}
\toprule
\textbf{Elemento} & \textbf{Valor} & \textbf{Fuente / Nota} \\
\midrule

Semilla generación sintética &
42 &
Scripts \texttt{scripts/generate\_data.py} \\

Semilla perturbación (\textit{drift}) &
7 &
\texttt{scripts/drift\_injector.py} \\

Tamaño ventana (\texttt{SAMPLE\_MAX}) &
1000 filas &
\texttt{.env}, \texttt{drift\_watch.py} \\

Periodo muestreo (\texttt{LOOP\_SECONDS}) &
30\,s &
\texttt{docker-compose.yml} (\texttt{drift-watch}) \\

Spark / PySpark &
3.5.1 (\texttt{pyspark==3.5.1}) &
\texttt{spark-base/Dockerfile} \\

MLflow &
2.14.x &
Servicio \texttt{mlflow} (pip install) \\

Airflow scheduler &
\texttt{apache/airflow:2.8.1 - python3.9} &
\texttt{docker-compose.yml} \\

Jenkins CI &
\texttt{jenkins/jenkins:lts} (2.462.x) &
\texttt{jenkins/Dockerfile} \\

Prometheus / Grafana &
\texttt{prom/prometheus:latest}, \texttt{grafana/grafana:latest} &
\texttt{docker-compose.yml} (pull 2025-05-10) \\

Python runtime (watcher / MLflow) &
\texttt{python:3.11-slim} &
Servicios \texttt{drift-watch}, \texttt{mlflow} \\
...
\vspace{3pt}
\footnotesize
Los valores reflejan la configuración usada en las 5 réplicas experimentales; cualquier cambio debe documentarse en MLflow junto con el \texttt{run\_id} correspondiente.
\end{table}
