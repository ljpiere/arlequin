...
El crecimiento exponencial en la generación de datos ha consolidado al aprendizaje automático como una herramienta clave para la toma de decisiones en diversos sectores. Sin embargo, los modelos de machine learning no son estáticos: su rendimiento se degrada cuando cambian las condiciones de entrada, fenómeno conocido como \textit{data drift} \citep{Sculley2015}. Este problema compromete la calidad de las predicciones y, por ende, la estabilidad operativa de los sistemas inteligentes.

Para abordar este desafío, el presente proyecto propone la implementación de un sistema automatizado de reentrenamiento integrado en un ciclo MLOps. Este enfoque adapta prácticas de ingeniería de software al ciclo de vida de los modelos, permitiendo su automatización, monitoreo y mantenimiento continuo \citep{Amershi2019}. El sistema opera monitoreando los datos en tiempo real; cuando detecta desviaciones estadísticas en su distribución (mediante pruebas como Kolmogorov-Smirnov o Kullback-Leibler), activa automáticamente un pipeline que reentrena y despliega un nuevo modelo.

Este flujo —implementado con herramientas como Jenkins, MLflow y Spark— garantiza que el sistema sea capaz de adaptarse sin intervención humana, asegurando precisión sostenida, trazabilidad y resiliencia en entornos Big Data.

La solución se valida en un entorno simulado, diseñado para replicar condiciones reales de operación, donde los datos de entrada (input) son flujos estructurados que emulan escenarios con y sin \textit{drift}, y la salida (output) es un modelo actualizado, validado empíricamente y registrado con sus métricas para futuras auditorías. Este enfoque apunta a transformar la gestión de modelos en producción, reduciendo la deuda técnica y mejorando la capacidad de respuesta ante entornos de datos dinámicos.

Para abordar estas brechas, este proyecto desarrolla una solución técnica que integra monitoreo continuo, detección estadística, reentrenamiento automatizado y trazabilidad del ciclo de vida del modelo.
...
A nivel regional, Latinoamérica ha mostrado una inversión sostenida en infraestructura tecnológica, superando los 6.360 millones de dólares en centros de datos en 2023, con una tasa de crecimiento proyectada del 7,95\% hasta 2029 \citep{Helmi2024}. Sin embargo, persisten retos estructurales en torno a la calidad del dato, la cultura organizacional y la escasa madurez en el uso de prácticas de MLOps. En Colombia, el 56\% de las bases de datos corporativas presenta deficiencias graves de completitud y exactitud \citep{Deyde2023}, lo que limita la eficacia de los sistemas de aprendizaje automático.

Frente a esta situación, el Gobierno colombiano ha lanzado en 2025 una política nacional de inteligencia artificial, destinando 480.000 millones de pesos a iniciativas de infraestructura y capacitación \citep{CONPES2025}. No obstante, esta inversión carece de acompañamiento técnico concreto para el despliegue de soluciones productivas automatizadas que aborden problemas como el \emph{data drift} de forma proactiva.

Desde el punto de vista económico, la ausencia de mecanismos automatizados para detectar y gestionar el \emph{data drift} puede acarrear pérdidas significativas. En el sector financiero colombiano, una caída del 3\% en la precisión de un modelo de riesgo crediticio se traduce en pérdidas que superan los 12 millones de dólares anuales \citep{Kim2018}. De igual forma, en el comercio electrónico, degradaciones menores en modelos de recomendación pueden afectar directamente las tasas de conversión y la rentabilidad.
...
El proyecto se concentró en la evaluación de un prototipo funcional basado en una arquitectura de Big Data previamente documentada en la literatura, evitando el diseño de una nueva infraestructura desde cero. Se utilizará como referencia una configuración de clúster pseudo-distribuido que emplea contenedores Docker y Docker Compose, replicando un sistema similar a HDFS y herramientas de procesamiento de datos como Apache Spark en un entorno controlado.
...
\midrule
E1 -- Base (sin drift)   & 0.835 (IC95\%: [\,\,]) & 0.84 & 0.05 (IC95\%: [\,\,]) & -- & -- \\
E2 -- Con drift inducido & 0.62\phantom{0} (IC95\%: [\,\,]) & 0.65 & 0.31 (IC95\%: [\,\,]) & 180 (IC95\%: [\,\,]) & -- \\
E3 -- Reentrenado        & 1.00\phantom{0} (IC95\%: [\,\,]) & 1.00 & 0.06 (IC95\%: [\,\,]) & -- & -- \\
\bottomrule
\end{tabular}
\end{table}

El escenario E2 evidenció una degradación significativa del desempeño (F1 $\downarrow$ 26\%), acompañada de un PSI superior al umbral de alerta (0.2). El sistema detectó el \textit{drift} en aproximadamente 3 minutos y activó el reentrenamiento, tras el cual el modelo recuperó su desempeño inicial (E3). Estas métricas confirman la capacidad del sistema para detectar y corregir desviaciones en tiempo casi real.
