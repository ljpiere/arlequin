...
Para mantener este objetivo enfocado en infraestructura (OE1), se limita el detalle estadístico a la \textbf{interfaz} que consume la arquitectura: el servicio de monitoreo expone valores-$p$ de KS/$\chi^2$ y el PSI como métrica agregada sobre \textit{scores}. Las \textbf{formulaciones y derivaciones completas} se documentan en el Marco de Referencia (Cap.~2) y en el diseño/validación de OE2 (Sec.~\ref{subsec:oe2} y Cap.~4), donde se definen hipótesis, umbrales (\(\alpha,\tau\)) y supuestos de operación. Aquí se declara únicamente el contrato operativo: las ventanas de referencia/reciente, los umbrales configurables en \texttt{.env} y el criterio OR de disparo ($p<\alpha$ o PSI $>\tau$) que alimenta la capa de control descrita en OE1.
...
\caption{Flujo OE2 optimizado y corregido.}
...
\begin{tikzpicture}[
  ...
  \node[data,   right=18mm of etl]        (hdfs)   {HDFS\\ventanas ref/rec};
  ...
  \node[box,    right=20mm of watch]      (jenkins){Jenkins\\\texttt{retrain-model}};

  \node[box,    right=20mm of jenkins]    (prom)   {Prometheus/Grafana};


  % --- Flechas (limpias, sin etiquetas superpuestas) ---
  \draw[arrow] (etl) -- node[above,pos=0.5,yshift=1mm] {parquet} (hdfs);

  \draw[arrow] (hdfs) -- node[right,pos=0.45,xshift=1.5mm] {muestras} (watch);

  % Variables de entorno sin etiqueta (para evitar solapamiento)
  \draw[arrow] (env) -- (watch);

  % Trigger Jenkins
  \draw[arrow] (watch) -- node[above,pos=0.55,yshift=1mm] {trigger} (jenkins);

  % Drift watcher → Prometheus/Grafana (sin /metrics)
  \draw[arrow] (watch) -- (prom);

  % Jenkins → MLflow (artefactos)
  \draw[arrow] (jenkins.south) |- node[right,pos=0.55,xshift=2mm] {artefactos} (mlflow.east);

  % Drift watcher → MLflow
  \draw[arrow] (watch) -- node[left,pos=0.5,xshift=-1mm] {CSV / MLflow} (mlflow);

\end{tikzpicture}

\caption{Contexto operativo del \textit{drift-watcher}: entradas desde el ETL y la configuración, y salidas hacia Prometheus, Jenkins y MLflow.}
\label{fig:oe2-context}
\end{figure}
...
La Figura~\ref{fig:oe3-metrics} resume cómo interactúan las métricas en OE3: el \texttt{drift\_watch.py} genera señales estadísticas (PSI, $p$-valores, TTFD) que alimentan Prometheus/Grafana; cuando se supera el umbral configurado, Jenkins ejecuta \texttt{retrain-model} y registra artefactos en MLflow junto con el TTR; finalmente, el job \texttt{eval-model} consume el modelo actualizado para calcular F1/AUC/Prec/Rec y publicar los resultados en MLflow y en los CSV de métricas. Este recuadro hace explícito el acoplamiento entre métricas de \textbf{detección} (PSI/$p$), \textbf{operación} (TTFD/TTR) y \textbf{desempeño} (F1/AUC), permitiendo rastrear cada réplica de OE3 de forma auditada.
...
\begin{tikzpicture}[
  ...
  % --- Flechas sin etiquetas largas ---
  \draw[arrow] (watch) -- (prom);
  \draw[arrow] (watch) |- (jenkins);
  \draw[arrow] (prom) |- (jenkins);
...
\end{tikzpicture}
\caption{Interacción de métricas en OE3: detección, operación y evaluación en el proceso automatizado de reentrenamiento.}
\label{fig:oe3-metrics}
\end{figure}
