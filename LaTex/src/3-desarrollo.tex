...
\paragraph{Diseño experimental.}  
El proceso experimental se organizó en \textbf{ciclos de evaluación de corta duración} (sub-minutales), garantizando un monitoreo continuo sin sobrecargar la cola de procesamiento.  
En cada iteración, el detector realiza tres operaciones consecutivas: lectura de los datos más recientes, contraste frente a la distribución de referencia y actualización de las métricas de observabilidad expuestas a Prometheus.  
Este enfoque permitió mantener un flujo constante de evaluación en tiempo casi real.

Para preservar la estabilidad estadística entre ciclos, se estableció un \textbf{límite superior de muestreo} (\textit{sampling cap}) que restringe el número máximo de filas por ventana.  
Dicha medida reduce la variabilidad entre ejecuciones y mejora la comparabilidad de los resultados.  
Las variables se procesaron según su tipo: las \textbf{numéricas} se analizaron mediante Kolmogorov–Smirnov, las \textbf{categóricas} con $\chi^2$, y la \textbf{salida del modelo (\textit{score})} con el índice PSI, mitigando posibles efectos de \textit{aliasing} o transformaciones no lineales.  
Este diseño posibilitó una detección ágil, reproducible y específica según la naturaleza de cada variable, manteniendo la consistencia de los resultados entre iteraciones sucesivas.

\paragraph{Indicadores y métricas (KPIs).}  
El comportamiento del detector se evaluó mediante un conjunto de \textbf{indicadores clave de rendimiento (KPIs)} que permiten cuantificar su precisión, eficiencia y estabilidad operativa:
\begin{itemize}\setlength\itemsep{2pt}
  \item \textbf{TTFD (Time-To-First-Detection):} intervalo entre la aparición del \textit{drift} y su detección estadísticamente válida, indicador de la rapidez de respuesta del sistema.
  \item \textbf{Tasa de falsas alarmas:} proporción de alertas generadas sin que exista un cambio real en la distribución, métrica esencial para calibrar la sensibilidad y especificidad del detector.
  \item \textbf{MTTR (Mean Time To Recovery):} tiempo medio requerido para restablecer la condición de normalidad tras una detección de \textit{drift}, reflejando la capacidad de recuperación del sistema.
  \item \textbf{Overhead computacional:} incremento en el uso de recursos (CPU, RAM, I/O) derivado de la ejecución del monitor, determinante para validar su viabilidad en entornos productivos.
  \item \textbf{Estabilidad del disparo:} grado en que el sistema evita el \textit{flapping}, es decir, la activación y desactivación repetida de alertas provocadas por fluctuaciones menores o ruido estadístico.
\end{itemize}

\subsection{Criterios de parametrización y robustez}

La calibración de $\alpha$, $\tau$ y del tamaño de ventana se realizó bajo un criterio de riesgo operativo: 
(i) fijar $\alpha=0{,}01$ para controlar la tasa de FP por columna, 
(ii) usar $\tau\in[0{,}2,0{,}3]$ como banda de alerta/alarma para PSI (convención industrial), y 
(iii) limitar \texttt{SAMPLE\_MAX} para estabilizar varianza del estimador por ciclo. 
El \emph{cooldown} se fijó para cubrir al menos un ciclo completo de reentrenamiento (evitando \emph{pile-ups}) y la \emph{clear streak} se ajustó para cubrir variabilidad intra-hora. 
Estos parámetros maximizan la sensibilidad sujeta a la restricción de estabilidad (\emph{no flapping}) y el presupuesto de cómputo.

Como verificación, se ejecutaron pruebas A/B de umbrales (banda $\alpha\in\{0{,}05,0{,}01,0{,}001\}$; $\tau\in\{0{,}1,0{,}2,0{,}3\}$) y de tamaño de ventana, registrando \emph{TTFD}, FP/FN y \emph{MTTR}. La combinación adoptada logró el mejor compromiso entre \emph{TTFD} y tasa de falsas alarmas sin penalizar la frecuencia de monitoreo.
...
\subsection{Realización técnica e instrumentación (scripts y métricas)}
...
(ii) un escenario \textbf{E2 (con \textit{drift} inducido)}, en el cual se introdujeron perturbaciones deliberadas sobre variables clave para simular tanto \textit{covariate shift} como \textit{concept shift}.  
