...
\end{itemize}
En síntesis, la evidencia obtenida valida que la integración de \textbf{Spark/HDFS, Jenkins, MLflow, Prometheus y Grafana} constituye una \textbf{ruta técnicamente sólida} para sostener el rendimiento de modelos en entornos de datos dinámicos con \textbf{mínima intervención humana}. Se sugiere, no obstante, revisar las conclusiones frente a los resultados de las métricas del modelo como reglas para decidir el despliegue final, integrando pasos de validación humana (\textit{human-in-the-loop}) cuando la criticidad del negocio lo requiera.
...
\paragraph{OE1 \;\textemdash\; Infraestructura escalable y monitorización continua}
\textbf{RQ1} (detección oportuna) y \textbf{H1} se apoyan en esta infraestructura: al exponer métricas y ejecutar el \textit{cooldown}, se logró mantener la tasa de falsas alarmas <1\,\% y un TTFD mediano de 270\,s (Sec.~\ref{subsec:eval-results}). La combinación de Spark/HDFS, Jenkins y Prometheus permite sostener la operación y responder de forma auditable.
\begin{itemize}\setlength\itemsep{2pt}
...
\paragraph{OE2 \;\textemdash\; Detección automática y reentrenamiento}
\textbf{RQ2} (recuperación del desempeño) y \textbf{H2} quedan validados: la diferencia E2 pre vs. post arrojó $p=5.07\times 10^{-3}$ y $|\delta|=0.54$ (Sec.~\ref{subsec:eval-results}), lo que demuestra que el gatillo KS/$\chi^2$/PSI más el reentrenamiento automático restablecen F1 a niveles no inferiores al baseline.
\begin{itemize}\setlength\itemsep{2pt}
...
\paragraph{OE3 \;\textemdash\; Validación experimental y rigor estadístico}
\textbf{RQ3} (estabilidad operativa) y \textbf{H3} se confirman: el diseño experimental (Sec.~\ref{subsec:eval-rq}) demostró que el pipeline mantiene TTR $\approx$102--103\,s y evita \textit{flapping} (36 ejecuciones por debajo del SLO), con evidencia estadística (Mann–Whitney, tamaños de efecto) registrada en Tabla~\ref{tab:mw-tests}.
\begin{itemize}\setlength\itemsep{2pt}
...
Los hallazgos se alinean con la evidencia sobre aprendizaje adaptativo ante \textit{drift} \citep{Gama2014,Zliobaite2016,Lu2019} y operacionalizan recomendaciones de MLOps (trazabilidad, versionado, automatización) \citep{Amershi2019}. En términos cuantitativos:
\begin{itemize}\setlength\itemsep{2pt}
  \item \textbf{TTFD/TTR:} la latencia de detección (\(\approx 270\) s) y el tiempo de recuperación (\(\approx 102\) s) mejoran los valores reportados en \cite{Amershi2019} y \cite{Chatterjee2023}, donde los reentrenamientos manuales rondan los 10–15 minutos.
  \item \textbf{F1-score:} la recuperación de F1 (de 0.811 a 0.817, sin cruzar el margen de no-inferioridad) supera la degradación residual observada en estudios de pipelines cerrados (\citealp{Bhatt2025}; \citealp{Berberi2025}), donde el F1 se estabiliza 3–4 puntos por debajo del baseline.
  \item \textbf{PSI/alertas:} la tasa de alertas <1\,\% y la sensibilidad PSI >0.2 coinciden con las guías en riesgo crediticio \citep{Siddiqi2012}, aportando evidencia replicable.
\end{itemize}
La contribución radica en \textbf{cerrar la brecha teoría–práctica} con una arquitectura abierta y cuantitativamente comparada: se documenta el stack, se entregan scripts reproducibles y se muestran resultados que mejoran los benchmarks existentes.
...
\begin{itemize}\setlength\itemsep{2pt}
  \item \textbf{Dominio simulado:} los resultados se obtienen sobre \textbf{transacciones bancarias sintéticas} para detección de fraude; otros casos de uso requieren generar variables y etiquetas específicas (p.\,ej., series de demanda o IoT) y recalibrar umbrales/SLAs antes de extrapolar el comportamiento del pipeline.
  \item \textbf{Infraestructura:} despliegue \textbf{single-host} sin alta disponibilidad (SPOF en NameNode/Prometheus) ni cifrado/RBAC formal; no se midió tolerancia a fallos ni comportamiento bajo autoscaling.
  \item \textbf{Metodología estadística:} detección basada en pruebas univariantes (KS/$\chi^2$/PSI) y \textbf{ventanas fijas}; no se evaluaron detectores multivariados ni sensibilidad a tamaños de ventana/alpha más conservadores.
