...
Las bases teóricas del proyecto se sustentan en diversos pilares conceptuales que explican el comportamiento de los modelos de \textit{machine learning} en entornos dinámicos, la necesidad de sistemas adaptativos y los enfoques tecnológicos subyacentes \citep{Kodakandla2024}. Para guiar el desarrollo arquitectónico, se propone el \textbf{modelo conceptual} de la Figura~\ref{fig:marco-conceptual}, el cual enlaza las clases de \textit{drift}, las técnicas de detección y los componentes MLOps que permiten reaccionar de forma reproducible.

\begin{figure}[htbp]
\centering
% Resizebox mantiene el diagrama ajustado al ancho de página
\resizebox{0.95\linewidth}{!}{
\begin{tikzpicture}[node distance=10mm and 18mm, >=latex]

% Estilo global de los bloques
\tikzstyle{block}=[rectangle, rounded corners=3pt, draw=black, thick, fill=white,
                   align=center, minimum width=28mm, minimum height=12mm]

% --- Nodos (Cajas) ---
\node[block] (data) {Data Streams \\ (Covariate / Concept)};
\node[block, right=of data] (detectors) {Detectores \\ KS / $\chi^2$ / PSI};
\node[block, right=of detectors] (policy) {Políticas \\ Edge + Cooldown};
\node[block, right=of policy] (ci) {CI/CD \\ (Jenkins)};
\node[block, right=of ci] (trace) {Trazabilidad \\ (MLflow / Prometheus)};

% --- Flechas de flujo principal (Arriba) ---
\draw[->, thick] (data) -- node[above, font=\footnotesize]{\texttt{SAMPLE\_MAX}} (detectors);
\draw[->, thick] (detectors) -- node[above, font=\footnotesize]{$p<\alpha$ o PSI $>\tau$} (policy);
\draw[->, thick] (policy) -- node[above, font=\footnotesize]{Trigger} (ci);
\draw[->, thick] (ci) -- node[above, font=\footnotesize]{Modelos + métricas} (trace);

% --- Flecha de Feedback Estilizada (Por debajo, estilo ortogonal) ---
% Explicación de la ruta:
% 1. Salir del sur de trace (trace.south)
% 2. Bajar 1.3cm relativo (++(0,-1.3))
% 3. Moverse horizontalmente hasta la coordenada X de 'data' (-|)
% 4. Subir al sur de 'data' (data.south)
\draw[->, thick, rounded corners=5pt] 
    (trace.south) -- ++(0,-1.3) 
    -| node[near start, above, font=\small] {Feedback Loop} 
    (data.south);

\end{tikzpicture}
}
\caption{Modelo conceptual: del \textit{drift} a la acción MLOps.}
\label{fig:marco-conceptual}
\end{figure}


Este marco explicita la taxonomía adoptada:
\begin{itemize}
  \item \textbf{Clase de \textit{drift}}: predominante \textit{covariate drift} con indicios parciales de \textit{concept drift}; articula la razón por la cual se seleccionan detectores univariantes.
  \item \textbf{Capa de detección}: pruebas KS/$\chi^2$/PSI operan sobre ventanas declarativas y alimentan políticas de activación.
  \item \textbf{Capa de control}: reglas \textit{edge-triggered} y \texttt{cooldown} estabilizan el disparo y conectan con orquestación Jenkins.
  \item \textbf{Capa de trazabilidad}: MLflow y Prometheus consolidan evidencia, habilitan auditorías y cierran el ciclo con retroalimentación.
\end{itemize}

\subsection{Machine Learning y Data Drift}
...
\begin{itemize}
    \item \textbf{Detección basada en estadísticos de distribución:} Compara la distribución de las variables (o la salida del modelo) en distintas ventanas de datos (\textit{sliding windows}), utilizando pruebas como Kolmogorov-Smirnov, $\chi^2$, divergencia de Kullback–Leibler y PSI. Estos enfoques son \textbf{no paramétricos} (salvo KL) y requieren supuestos mínimos: KS no impone forma de la distribución y contrasta CDFs; $\chi^2$ compara frecuencias categóricas; PSI resume la divergencia en un valor único tras un \textit{binning} explícito, lo que lo hace interpretable para operaciones. Su sensibilidad depende del tamaño de ventana y de la elección de umbrales ($\alpha$, $\tau_{\text{PSI}}$), que deben calibrarse por dominio para equilibrar falsos positivos y detección temprana.
...
La elección de pruebas como Kolmogorov-Smirnov, Chi-cuadrado o el Population Stability Index (PSI) responde a su amplia utilización en la literatura de \textit{drift detection} y a su complementariedad. El test de Kolmogorov-Smirnov es sensible a variaciones en la forma de la distribución (media, varianza o asimetría) de variables continuas, mientras que la prueba Chi-cuadrado resulta idónea para identificar cambios en frecuencias categóricas \citep{Sethi2017}. Por su parte, el PSI —estándar en riesgo crediticio— es usable en otros dominios siempre que se definan \textit{bins} y umbrales operativos acordes al caso de uso (p.\,ej., umbrales más bajos en contextos volátiles o con alta estacionalidad). Estas métricas permiten cubrir distintos tipos de variables y escenarios de \textit{drift}, equilibrando sensibilidad estadística y aplicabilidad práctica \citep{Gama2014, Zliobaite2016}. En sistemas productivos se recomienda complementar su uso con control de multiplicidad (Holm/BH) y guías de gobernanza sobre tamaño de ventana y frecuencia de muestreo para mitigar falsos positivos.

\paragraph{PSI: aplicabilidad y cautelas.}
El \textbf{Population Stability Index} resume en un solo valor la divergencia entre distribuciones discretizadas y, por ello, es atractivo más allá del ámbito bancario: se ha usado en \textit{e-commerce}, telecom y manufactura para vigilar \textit{scores} o variables derivadas. Su validez depende del \textit{binning} (cuantiles o intervalos de negocio), de contar con soporte mínimo por cajón (5–10\,\% recomendado) y de calibrar umbrales por dominio (\(\tau\approx 0.1/0.2/0.3\) son guías en riesgo, pero pueden ajustarse en dominios con mayor volatilidad). Como no es una prueba exacta, conviene reportar PSI junto con KS/$\chi^2$ y usarlo como métrica de magnitud, no como único disparador cuando la cardinalidad o el \textit{binning} son inestables.

\section{Contraste crítico y criterios de selección}
Para maximizar validez práctica en operación, la selección del detector debe ponderar cinco criterios principales.
\begin{itemize}
  \item \textbf{Cobertura de variables:} identificar si el detector soporta variables continuas, categóricas o mixtas (p.\,ej., KS para continuas, $\chi^2$ para categóricas, PSI con \textit{binning}).
  \item \textbf{Modo de operación y latencia:} distinguir entre enfoques por ventanas \textit{batch}/deslizantes y detectores en flujo (\textit{online}) que reaccionan evento a evento (ADWIN, EDDM).
  \item \textbf{Costo computacional y memoria:} evaluar si el método implica conteos simples, ordenamientos ($\mathcal{O}(n\log n)$) o estructuras adaptativas, lo cual impacta su escalabilidad en producción.
  \item \textbf{Calibración y gobernanza:} priorizar métricas con umbrales operativos claros (PSI, $\chi^2$) y documentar las condiciones mínimas (tamaño de ventana, suavizado) para evitar falsos positivos.
  \item \textbf{Extensión multivariada e interpretabilidad:} determinar si basta con combinar pruebas univariantes controlando FDR o si se requieren detectores multivariados adicionales, manteniendo trazabilidad para auditorías.
\end{itemize}
El Cuadro~\ref{tab:comparativa-detectores} resume estos criterios y remite al anexo técnico para las fórmulas e implicaciones completas.

\section{Cuadro comparativo de detectores}
...
Continua (univar.) &
Ventanas (batch) &
Medio ($\mathcal{O}(n\log n)$ por ventana) \\

$\chi^2$ de independencia &
Categórica &
Ventanas (batch) &
Bajo ($\mathcal{O}(k)$; conteos) \\

Kullback--Leibler (KL) &
Num./cat. discretizada &
Ventanas (batch) &
Medio ($\mathcal{O}(k)$; requiere suavizado) \\

Population Stability Index (PSI) &
Num./cat. (con binning) &
Ventanas (batch) &
Bajo ($\mathcal{O}(\text{bins})$) \\

EDDM \citep{BaenaGarcia2006} &
Métrica de error (cualquiera) &
Flujo (\textit{online}) &
Bajo ($\mathcal{O}(1)$ por evento) \\

ADWIN \citep{Bifet2010} &
Métrica de error (cualquiera) &
Flujo (\textit{online}) &
Bajo/medio (amort. $\mathcal{O}(\log n)$) \\
...
\subsection{MLOps y Automatización del Ciclo de Vida de los Modelos}
...
\subsection{Big Data y Tecnologías Emergentes}
...
\subsection{Adaptación y Aprendizaje en Línea}
...
\section{Evaluación de Sistemas de Detección y Reentrenamiento}
...
En total, se han revisado 19 estudios que abordan la detección del \textit{data drift} y la automatización del reentrenamiento. De ellos, \textbf{11 se apoyan principalmente en pruebas estadísticas univariadas/multivariadas} (KS, $\chi^2$, PSI, KL); \textbf{5 proponen esquemas híbridos} que combinan tests con clasificadores de cambio o detectores \textit{online}; y \textbf{3 usan modelos supervisados} como discriminadores entre ventanas de datos. Esta distribución confirma que las pruebas estadísticas siguen siendo el enfoque predominante, pero crece el interés en combinar señales para aumentar sensibilidad y robustez. A pesar de estos avances, persiste una brecha en la integración automatizada y escalable de dichas técnicas en pipelines productivos. Esta investigación se propone contribuir a ese vacío, desarrollando un sistema replicable que permita gestionar el \textit{data drift} de manera proactiva, automatizada y trazable en contextos reales.

\begin{table}[htbp]
\centering
\caption{Clasificación del corpus revisado por enfoque de detección.}
\label{tab:corpus-detectors}
\small
\begin{tabular}{l p{6.8cm} c}
\toprule
\textbf{Enfoque} & \textbf{Referencias representativas} & \textbf{\#} \\
\midrule
Pruebas estadísticas (KS/$\chi^2$/PSI/KL) & 
\citep{Gama2014,Lu2019,Chawla2021,Sethi2017,Nguyen2023} & 11 \\

Híbridos (tests + detectores \textit{online}/clasificadores de cambio) & 
\citep{BaenaGarcia2006,Bifet2010,Singh2023} & 5 \\

Supervisados como discriminadores de ventana & 
\citep{Chatterjee2023,Agarwal2023} & 3 \\
\bottomrule
\end{tabular}
...
Diversas plataformas empresariales ofrecen actualmente capacidades para automatizar el monitoreo de modelos \citep{Bhatt2025}, la detección de \textit{data drift} y el reentrenamiento en entornos reales \citep{Berberi2025,Lwakatare2020}. Entre las más destacadas se encuentran AWS SageMaker, Google Vertex AI y Azure Machine Learning. Adicionalmente, en la industria han surgido servicios especializados en observabilidad de ML como \textbf{Fiddler}, \textbf{Arize AI} y \textbf{Evidently AI} (como servicio gestionado), los cuales ofrecen capacidades profundas de \textit{root cause analysis} y monitoreo de \textit{drift} que complementan a los proveedores de nube generalistas. En el caso de AWS SageMaker, la herramienta \texttt{Model Monitor} permite evaluar continuamente los datos de entrada y salida del modelo, generando alertas ante desviaciones relevantes, lo cual ha sido documentado en estudios de referencia sobre plataformas MLOps \citep{Berberi2025}. Sin embargo, su carácter cerrado y dependiente del ecosistema AWS limita su personalización para dominios con métricas particulares \citep{Bhatt2025}.

Google Vertex AI también proporciona funcionalidades avanzadas de detección de \textit{drift} y reentrenamiento, pero su fuerte integración con AutoML y su arquitectura opaca dificultan la intervención directa en el pipeline \citep{Berberi2025}. Por su parte, Azure Machine Learning incluye herramientas para CI/CD y monitoreo con Azure Monitor; no obstante, las evaluaciones comparativas señalan restricciones en extensibilidad metodológica y adaptabilidad a casos de uso altamente personalizados \citep{Bhatt2025,Lopez2022}.

% \paragraph{Capacidades de Azure Machine Learning y Azure Monitor para pipelines MLOps}

Microsoft Azure ofrece un ecosistema integrado para la gestión completa del ciclo de vida de los modelos de \textit{machine learning}. En particular, \textbf{Azure Machine Learning (Azure ML)} proporciona un entorno escalable para entrenamiento, despliegue y monitoreo continuo, con énfasis en reproducibilidad y trazabilidad documentado por estudios recientes sobre MLOps en la nube \citep{Berberi2025,Bhatt2025}. Sus principales capacidades incluyen:

\begin{itemize}
    \item \textbf{Automatización y CI/CD:} permite definir \textit{pipelines} de entrenamiento y despliegue continuo integrados con Azure DevOps y GitHub Actions, reduciendo la intervención manual y garantizando control de versiones sobre datos, modelos y código \citep{Berberi2025,Bhatt2025,AzureML2023}.
    \item \textbf{Detección de \textit{drift}:} incluye módulos nativos de \textit{Data Drift Monitoring} que comparan distribuciones históricas con datos recientes mediante métricas estadísticas y alertas configurables, alineándose con las recomendaciones de automatización reportadas en \citep{Bhatt2025}. 
    \item \textbf{Monitoreo operacional:} la integración con \textbf{Azure Monitor} y \textbf{Application Insights} facilita la observabilidad de modelos en producción, centralizando métricas de inferencia, consumo de recursos y desempeño \citep{Berberi2025,AzureMonitor2022}.
    \item \textbf{Escalabilidad en la nube:} gracias a la compatibilidad con clústeres de cómputo administrados (CPU/GPU), Azure ML soporta cargas intensivas y permite ajustar dinámicamente los recursos de entrenamiento e inferencia en función de la demanda \citep{Berberi2025}.
\end{itemize}

Estas capacidades posicionan a Azure como una alternativa robusta frente a SageMaker y Vertex AI, con la ventaja de su integración nativa con el ecosistema empresarial de Microsoft \citep{Berberi2025}. No obstante, la literatura técnica también señala que su carácter parcialmente cerrado y la dependencia de servicios gestionados pueden limitar la flexibilidad metodológica para investigadores que requieren un control granular de cada etapa del pipeline \citep{Bhatt2025,Lopez2022, Xu2022}.
