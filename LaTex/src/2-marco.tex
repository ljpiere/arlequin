...
\tikzstyle{block}=[rectangle, rounded corners=3pt, draw=black, thick, fill=white, align=center, minimum width=30mm, minimum height=10mm]
\node[block] (data) {Data Streams \\ (Covariate / Concept)};
\node[block, right=25mm of data] (detectors) {Detectores \\ KS / $\chi^2$ / PSI};
\node[block, right=25mm of detectors] (policy) {Políticas \\ Edge + Cooldown};
\node[block, right=25mm of policy] (ci) {CI/CD \\ (Jenkins)};
\node[block, right=25mm of ci] (trace) {Trazabilidad \\ (MLflow / Prometheus)};
\draw[->, thick] (data) -- node[above]{\small Ventanas $\texttt{SAMPLE\_MAX}$}(detectors);
\draw[->, thick] (detectors) -- node[above]{\small $p<\alpha$ o $\mathrm{PSI}>\tau$}(policy);
\draw[->, thick] (policy) -- node[above]{\small Trigger}(ci);
\draw[->, thick] (ci) -- node[above]{\small Modelos + métricas}(trace);
\draw[->, thick, bend right=35] (trace.south) to node[below]{\small Feedback}(data.south);
\end{tikzpicture}}
...
Para maximizar validez práctica en operación, la selección del detector debe ponderar cinco criterios principales \citep{Lu2019, Gama2014, Bifet2010}; los desarrollos matemáticos y comparativas extendidas se trasladan al Anexo~\ref{ann:detectors}.
\begin{itemize}
...
\midrule
Pruebas estadísticas (KS/$\chi^2$/PSI/KL) & \citep{Gama2014,Lu2019,Chawla2021,Sethi2017,Nguyen2023} & 11 \\
Híbridos (tests + detectores \textit{online}/clasificadores de cambio) & \citep{BaenaGarcia2006,Bifet2010,Singh2023} & 5 \\
Supervisados como discriminadores de ventana & \citep{Chatterjee2023,Agarwal2023} & 3 \\
\bottomrule
\end{tabular}
\\[2pt]
\footnotesize El número total corresponde al corpus revisado; la tabla lista ejemplos representativos de cada categoría y no agota las 19 referencias. El detalle por estudio se documenta en el repositorio de lectura sistemática.
\end{table}
